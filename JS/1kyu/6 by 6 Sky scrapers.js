function solvePuzzle(clues) {
  function getRow(n) {
    const dict = { 0: 5, 1: 4, 2: 3, 3: 2, 4: 1, 5: 0 };
    if (n < 6) {
  ...
  function raidRows(currentNRow, currentQ, n) {
    ...
    const alternate = alternateRow(currentNRow, currentQ);
    const rowB = getRow(alternate);
  ...
  function genPermutesForPeaks() {
    ...
    const allPermutes = genPermutes([1, 2, 3, 4, 5, 6], 6)
  ...
  function runAllRows() {
    const prevSolve = JSON.stringify(toSolve);
  ...
  const tmp = [1, 2, 3, 4, 5, 6],
    aOfT = [tmp.concat(), tmp.concat(), tmp.concat(), tmp.concat(), tmp.concat(), tmp.concat()],
