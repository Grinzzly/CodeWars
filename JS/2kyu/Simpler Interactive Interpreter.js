...
Interpreter.prototype.assign = function (x, y, scope) {
    let value = this.extractValue(y, scope);
    if (scope.x !== void 0) {
...
Interpreter.prototype.exec = function (syntaxTree, scope) {
    scope = scope || {};
    let name = syntaxTree[0], params, _this = this;
...
Interpreter.prototype.input = function (expr) {
    let syntax, result, tokens = this.tokenize(expr);
    if (!tokens.length) {
        return '';
    }
...
Interpreter.prototype.functionParser = function (tokens) {
    let index = tokens.indexOf('=>'), paramObj = {}, params = [], fnName = tokens[1];
    ...
    let result = this.expressionParser(tokens.slice(index + 1));
    let syntaxTree = result[0], varList = result[1];
...
Interpreter.prototype.expressionParser = function (tokens) {
    let SPACE = {}, params = [], operatorStack = [], dataStack = [SPACE], expressionFlag = true, lValue, rValue, operator, vars = {};
    tokens = tokens.slice();
    tokens.push(')');
    tokens.unshift('(');
    ...
    while (tokens.length) {
        ...
        if (next === ')') {
            ...
            params = [next];
            for (let i in this.functions[next].params) {
